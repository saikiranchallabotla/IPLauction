...
// Format: YYYY-MM-DD. IPL typically starts around March 22. Using March 22 as default
...
const FANTASY_CACHE_VERSION = 11;
...
async function fetchFromIPLFantasyPublic() {
    ...
    for (const entry of gamedayEntries) {
        ...
        // Stop as soon as we've found more matches than physically possible this season.
        // Applied in BOTH probing mode AND fixture mode to prevent stale prior-season
        // data (the API reuses tourgamedayId values across seasons) from slipping through.
        ...
        const effectiveMax = schedulePlayedCount > 0
            ...
            : maxPossibleMatches;
        if (newMatches.length >= effectiveMax) {
            console.log(`IPL Fantasy Public: reached max matches (${effectiveMax}, schedule=${schedulePlayedCount}) for IPL ${IPL_SEASON_YEAR}, stopping`);
            break;
        }

        // If we already hit an unplayed match boundary (probing mode only), stop.
        // The same tourgamedayId can return stale data from a prior IPL season because
        // the API reuses gameday IDs across seasons. Once an unplayed match is found,
    ...
    // truncate to maxPossibleMatches. This catches stale prior-season data that was
    // cached with the wrong seasonYear, or fixture-mode over-fetches.
    if (allMatches.length > maxPossibleMatches) {
        console.log(`IPL Fantasy Public: total ${allMatches.length} matches exceeds max possible ${maxPossibleMatches} — truncating to ${maxPossibleMatches}`);
        allMatches = allMatches.slice(0, maxPossibleMatches);
    }
    ...
    // 2. Undated probed matches: if we have an IPL Stats schedule, discard them as stale
    //    prior-season data (the API reuses tourgamedayId across seasons). Without a schedule,
    //    cap undated matches to remaining slots within maxPossibleMatches.
    const hasSchedule = iplStatsSchedule.length > 0;
    const datedMatches = allMatches.filter(m => {
        if (!m.matchDate) return false; // handle undated separately
        const d = new Date(m.matchDate);
        return !isNaN(d.getTime()) && d >= seasonStart;
    });
    const undatedMatches = allMatches.filter(m => !m.matchDate);
    let keptUndated;
    if (hasSchedule) {
        // Schedule is available — undated matches couldn't be enriched, meaning they're
        // stale prior-season data not present in the current season's schedule.
        keptUndated = [];
        for (const m of undatedMatches) {
        ...
        // No schedule — fall back to slot-based cap to limit undated matches.
        const remainingSlots = Math.max(0, maxPossibleMatches - datedMatches.length);
        keptUndated = undatedMatches.slice(0, remainingSlots);
    }
    const validMatches = [...datedMatches, ...keptUndated];
    const removed = allMatches.length - validMatches.length;
    if (removed > 0) {
        console.log(`IPL Fantasy Public: removed ${removed} stale matches (not in IPL ${IPL_SEASON_YEAR} or before season start)`);
    }
...
async function fetchFromCricAPI() {
    ...
    // Background: enrich matches with scorecard data from Cricbuzz/ESPN
    enrichMatchesWithScorecards().catch(err =>
        console.warn('Background scorecard enrichment failed:', err.message)
    );
...
// After the main fantasy fetch (especially ipl_public which only returns points),
// try to fetch detailed scorecard data from IPL Stats (iplt20.com - free, no auth)
// and attach it to the cached matches so the client can display scorecards.

let scorecardEnrichmentInProgress = false;

async function enrichMatchesWithScorecards() {
    if (!fantasyCache?.matches?.length) return;
    if (scorecardEnrichmentInProgress) return;
    scorecardEnrichmentInProgress = true;
    ...
    try {
        ...
        if (matchesNeedingScorecard.length === 0) {
            console.log('Scorecard enrichment: all matches already have scorecards');
            return;
        }
        ...
        const stillNeeding = matchesNeedingScorecard.filter(m => !m.scorecard || m.scorecard.length === 0);
        if (stillNeeding.length > 0) {
            ...
            try {
                const [liveRes, recentRes] = await Promise.allSettled([
                    fetch('https://www.cricbuzz.com/api/cricket-match/live', { headers: cbHeaders }),
                    fetch('https://www.cricbuzz.com/api/cricket-match/recent', { headers: cbHeaders }),
                ]);
                const cbMatchMap = new Map();
                for (const result of [liveRes, recentRes]) {
                        ...
                        for (const typeGroup of (data.typeMatches || [])) {
                            for (const seriesGroup of (typeGroup.seriesMatches || [])) {
                                ...
                                for (const m of (wrapper.matches || [])) {
                                    const info = m.matchInfo || m;
                                    const matchId = String(info.matchId || info.id || '');
                                    if (matchId) cbMatchMap.set(matchId, { info });
                                }
                            }
                        }
                    } catch (_) {}
                }
                for (const match of stillNeeding) {
                    const matchNameLower = (match.matchName || '').toLowerCase();
                    let cbMatchId = null;
                    for (const [mId, cb] of cbMatchMap) {
                        const cbDesc = (cb.info.matchDesc || '').toLowerCase();
                        const cbNum = cbDesc.match(/match\s*(\d+)/)?.[1];
                        const fcNum = matchNameLower.match(/match\s*(\d+)/)?.[1];
                        if (cbNum && fcNum && cbNum === fcNum) { cbMatchId = mId; break; }
                    }
                    if (!cbMatchId) continue;
                    try {
                        const scRes = await fetch(`https://www.cricbuzz.com/api/cricket-scorecard/${cbMatchId}`, { headers: cbHeaders });
                        ...
                        if (innings.length > 0) {
                            match.scorecard = innings;
                            enriched++;
                            console.log(`  Enriched scorecard (Cricbuzz) for ${match.matchName} (${innings.length} innings)`);
                        }
                    } catch (_) {}
                }
            } catch (_) {}
        ...
        if (enriched > 0) {
            console.log(`Scorecard enrichment: enriched ${enriched} matches`);
...
// Extract scorecard display data from CricAPI's scorecard format
function extractCricAPIScorecardData(scData) {
    const innings = [];
    const scorecard = scData?.scorecard || scData?.data?.scorecard || [];
    // CricAPI also provides score array with team totals
    const scoreArr = scData?.score || scData?.data?.score || [];

    for (let i = 0; i < scorecard.length; i++) {
        const inning = scorecard[i];
        // Determine team name from score array or batting data
        let teamName = '';
        if (scoreArr[i]) {
            teamName = scoreArr[i].inning || '';
            // "Team Name Inning 1" -> "Team Name"
            teamName = teamName.replace(/\s+inning(s)?\s*\d*$/i, '').trim();
        }
        const totalRuns = scoreArr[i]?.r ?? 0;
        const totalWickets = scoreArr[i]?.w ?? 0;
        const totalOvers = String(scoreArr[i]?.o ?? '0');
        ...
        for (const b of (inning.batting || [])) {
            ...
            if (!name) continue;
            const dcode = String(b['dismissal-code'] || b.dismissal || '').toLowerCase();
            const isNotOut = !dcode || dcode.includes('not out') || dcode === 'not-out' || dcode === 'batting';
            const r = parseInt(b.r ?? 0), bl = parseInt(b.b ?? 0);
            batsmen.push({
                name, runs: r, balls: bl,
                fours: parseInt(b['4s'] ?? 0), sixes: parseInt(b['6s'] ?? 0), isNotOut,
                strikeRate: bl > 0 ? ((r / bl) * 100).toFixed(1) : '0.0'
            });
        }
        batsmen.sort((a, b) => (b.isNotOut - a.isNotOut) || (b.runs - a.runs));
        ...
        for (const bw of (inning.bowling || [])) {
            const name = bw.bowler;
            if (!name) continue;
            const overs = String(bw.o ?? '0');
            const r = parseInt(bw.r ?? 0);
            bowlers.push({
                name, overs, maidens: parseInt(bw.m ?? 0),
                runs: r, wickets: parseInt(bw.w ?? 0),
...
async function fetchScorecardFromCricAPI() {
    ...
    // Get match list for the series
    const seriesRes = await fetch(
        `https://api.cricapi.com/v1/series_info?apikey=${apiKey}&id=${seriesId}`
    );
    const seriesData = await seriesRes.json();
    if (seriesData.status !== 'success') throw new Error(`CricAPI series_info: ${JSON.stringify(seriesData.info || 'error')}`);

    const matchList = seriesData.data?.matchList || seriesData.data?.matches || [];
    const now = Date.now();
    const oneDayAgo = now - 24 * 60 * 60 * 1000;

    // Filter to live + recently completed matches
    const candidates = [];
    for (const match of matchList) {
        const statusStr = String(match.status || '').toLowerCase();
        ...
        const isCompleted = match.matchEnded ||
            ...
            statusStr.includes('draw') || statusStr.includes('tie');
        if (!isLive && !isCompleted) continue;
        const matchDate = match.dateTimeGMT ? new Date(match.dateTimeGMT).getTime() : 0;
        if (!isLive && matchDate && matchDate < oneDayAgo) continue;
        candidates.push({ ...match, isLive, isCompleted, matchDate });
    }

    // Sort: live first, then most recent
    candidates.sort((a, b) => (b.isLive - a.isLive) || (b.matchDate - a.matchDate));
    // Limit to avoid burning API quota — live matches + last 2 completed
    const toFetch = [
        ...candidates.filter(c => c.isLive),
        ...candidates.filter(c => !c.isLive).slice(0, 2)
    ];

    if (toFetch.length === 0) throw new Error('CricAPI: no live/recent IPL matches');
    ...
    for (const match of toFetch) {
        try {
            ...
            const scData = await scRes.json();
            if (scData.status !== 'success' || !scData.data?.scorecard?.length) continue;

            const innings = extractCricAPIScorecardData(scData.data);
            if (innings.length > 0) {
                const statusStr = String(match.status || '').toLowerCase();
                const isLive = match.isLive;
                const t1 = scData.data.teams?.[0] || match.teams?.[0] || '';
                const t2 = scData.data.teams?.[1] || match.teams?.[1] || '';
                scorecards.push({
                    matchName: scData.data.name || match.name || `${t1} vs ${t2}`,
            ...
            console.warn(`CricAPI scorecard fetch error for ${match.id}: ${err.message}`);
        }
    }
    return scorecards;
}
...
app.get('/api/match-scorecards', async (req, res) => {
    try {
        ...
        // CricAPI is primary: proper API with key, no scraping/blocking issues
        const sources = [
            { name: 'CricAPI', fn: fetchScorecardFromCricAPI },
