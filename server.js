...
io.on('connection', (socket) => {
    ...
    socket.on('updateBid', ({ amount, teamId }) => {
        ...
        // Budget check (round both sides to avoid floating-point issues)
        if (Math.round(amount * 100) > Math.round(team.budget * 100)) return;
    ...
    socket.on('tradePlayer', ({ playerName, fromTeamId, toTeamId, tradePrice }) => {
        ...
        // Check target team budget (round both sides to avoid floating-point issues)
        if (Math.round(price * 100) > Math.round(toTeam.budget * 100)) return;
    ...
    socket.on('addPlayer', ({ name, role, country, previousTeam, basePrice, assignToTeamId }) => {
        ...
        if (assignToTeamId) {
            ...
            if (team) {
                ...
                if (Math.round(price * 100) <= Math.round(team.budget * 100) && (team.players || []).length < 16) {
